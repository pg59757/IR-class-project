...
class TermDocumentMatrix:
    ...
    def __init__(
        self,
        preprocessor: Optional[Preprocessor] = None,    #Text preprocessor to use for tokenisation, stemming, etc.
        fields: Optional[list[str]] = None,             #Document fields to index. Defaults to ["title", "abstract"].
        mode: MatrixMode = "binary",                    #Cell weighting scheme.
    ):
    ...
    def boolean_search(self, query: str) -> list[TDMResult]:
        """
        Executes a boolean query on the term‑document matrix using bitwise ops.
        Supports AND/OR/NOT with correct precedence and implicit AND.
        Returns TDMResult entries sorted by matched‑term score.
        """
    ...
    def get_term_vector(self, term: str) -> Optional[np.ndarray]:
        """
        Returns the row vector for a term after preprocessing.
        Returns a 1‑D array of length |documents|, or None if not in the vocabulary.
        """
    ...
    def get_document_vector(self, doc_id: int) -> Optional[np.ndarray]:
        """
        Returns the column vector for a document.
        Returns a 1‑D array of length |vocabulary|, or None if the document is missing.
        """
