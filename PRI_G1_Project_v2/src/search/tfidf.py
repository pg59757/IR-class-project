...
class TFIDFEngine:
    ...
    def __init__(
        self,
        preprocessor: Optional[Preprocessor] = None,    #Preprocessor
        fields: Optional[list] = None,                  #fields to index (default: title + abstract)
        tf_scheme: TFScheme = "log",                    #"raw" | "log" | "boolean"
        use_sklearn: bool = False,                      #bool  — use sklearn backend if True
    ):
    ...
    def similarity_matrix(self) -> tuple[np.ndarray, list[int]]:
        """
        Computes an N×N cosine‑similarity matrix for all indexed documents.
        Works with both custom and sklearn backends. Returns the matrix and doc ID order.
        """
