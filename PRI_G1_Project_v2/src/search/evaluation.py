...
class RelevanceMetrics:
    ...
    method: str             #Engine label (e.g. \"custom\", \"sklearn\").
    precision_at_k: float   #Fraction of top-K results that are relevant.
    recall_at_k: float      #Fraction of known relevant docs retrieved in top-K.
    avg_precision: float    #Average precision (area under precision-recall curve).
    k: int                  #Cutoff used.
...
class PerformanceEvaluator:
    ...
    def benchmark_indexing(self, documents: list[dict]) -> list[IndexingStats]:
        """
        Builds stemming and lemmatisation inverted indexes, measuring time and peak memory.
        Returns two IndexingStats entries (stemming + lemmatisation).
        """
        results = []
    ...
    def benchmark_tfidf_indexing(self, documents: list[dict]) -> list[IndexingStats]:
        """
        Benchmarks TF‑IDF index building for stemming and lemmatisation.
        Returns IndexingStats for custom and sklearn TF‑IDF.
        """
        results = []
        configs = [
    ...
    def benchmark_queries(
        ...
        """
        Measures response time for a list of queries against a built engine.
        Returns one QueryStats entry per query.
        """
        results = []
        for q in queries:
            t0 = time.perf_counter()
    ...
    def compare_rankings(
        ...
        """
        Compares top‑K rankings between custom and sklearn TF‑IDF for each query.
        Returns a list of RankingComparison entries.
        """
    ...
    def evaluate_relevance(
        ...
        """
        Computes P@K, R@K and Average Precision for each query in the qrels.
        Relevance is based on title substring matching. Returns RelevanceMetrics per query.
        """
        results = []
    ...
    def full_report(
        self,
        documents: list[dict],                          #Corpus to use for indexing and search.
        queries: Optional[list[str]] = None,            #Optional list of queries for response-time and ranking-comparison benchmarks.
        qrels: Optional[dict[str, list[str]]] = None,   #Optional relevance judgements for P@K/R@K/MAP evaluation. Defaults to DEFAULT_QRELS if not provided.
    ) -> PerformanceReport:
        """
        Runs all benchmarks and returns a consolidated PerformanceReport.
        """
        if queries is None:
...
def build_index_in_batches(
    ...
    """
    Builds an inverted index incrementally in batches to reduce memory usage.
    Returns a fully constructed InvertedIndex.
    """
