...
from src.search.text_pipeline import Preprocessor, make_stemming_preprocessor
...
class ClassificationResult:
    ...
    """
    predicted_category: str             #The top predicted research area.
    confidence: float                   #Probability of the top category (0.0–1.0).
    probabilities: dict[str, float]     #Dict mapping each category to its probability.
    text_used: str = ""                 #The text that was classified (title + abstract).
...
class TrainingReport:
    ...
    """
    num_documents: int                                                  #Total documents used for training.
    category_counts: dict[str, int]                                     #How many documents per category.
    cross_val_accuracy: float                                           #Mean accuracy from 3-fold cross-validation (if run).
    vocabulary_size: int                                                #Number of TF-IDF features used.
    categories: list[str] = field(default_factory=list)                 #List of category labels.
    per_category_metrics: dict[str, dict] = field(default_factory=dict) #Precision, recall and F1 per category (REQ-B44).
    macro_avg: dict[str, float] = field(default_factory=dict)           #Macro-averaged precision, recall, F1.
    weighted_avg: dict[str, float] = field(default_factory=dict)        #Weighted-averaged precision, recall, F1.
...
def derive_label(text: str) -> str:
    """
    Assigns a category to a document by counting keyword matches.
    Checks which category’s keywords appear in the lowercased text and returns the best match,
    or "other" if none exceed the threshold.
    """
...
class DocumentClassifier:
    ...
    def __init__(
        self,
        preprocessor: Optional[Preprocessor] = None,    #Text preprocessor (stemming / lemmatisation).
        alpha: float = 1.0,                             #Laplace smoothing parameter for Naïve Bayes (default 1.0).
        max_features: int = 5000,                       #Maximum TF-IDF vocabulary size (default 5000).
        fields: Optional[list[str]] = None,             #Document fields to use as text. Default: title + abstract.
        run_cross_validation: bool = False,             #If True, perform 3-fold cross-validation and report mean accuracy.
    ...
    def train(self, documents: list[dict]) -> TrainingReport:
        """
        Derives labels and trains the Naive Bayes classifier:
        1) extract text, 2) assign category via keywords, 3) TF‑IDF vectorise, 4) fit MultinomialNB,
        5) optional 3‑fold cross‑validation.
    ...
    def classify(self, text: str) -> ClassificationResult:
        """
        Predicts the research category for a raw text string.
        Returns a ClassificationResult with category + probabilities.
        Raises RuntimeError if the classifier has not been trained.
        """
        self._check_trained()
    ...
    def classify_document(self, doc: dict) -> ClassificationResult:
        """
        Classifies a document dict by extracting its text fields.
    ...
    def classify_batch(self, documents: list[dict]) -> list[ClassificationResult]:
        """
        Classifies a list of documents in a single vectorisation pass (efficient).
    ...
    def get_top_features(self, category: str, n: int = 10) -> list[tuple[str, float]]:
        """
        Returns the top N TF‑IDF feature weights for a category.
        Useful for explaining classification decisions. Returns (term, weight) pairs.
        """
        self._check_trained()
